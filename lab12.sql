...
--List the names of pizzeria and its customer which have at least one customer 
SELECT
    person.*,
    frequents.pizzeria
FROM
    person
    INNER JOIN frequents ON person.name = frequents.name;

--List the names of all pizzeria including those who have no customer
SELECT
    frequents.pizzeria
FROM
    person
    RIGHT OUTER JOIN frequents ON frequents.name = person.name;
